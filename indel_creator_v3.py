...
for i in range(1, 601):
    ...
    generate_config(updated_json_data, indel_input_path)

def rename_indel(indel_input_path):
    ...
    for i in control_files:
        original_path = os.path.join(indel_input_path, i)
        temp_path = os.path.join(indel_input_path, 'control.txt')
...
def move_to_indel_output(indel_input_path, output_directory):
...
def graph_correct_outputs(tree_output_path, user_data, tq_dist_path, graph_path):
    ...
    vs the incorrect ones. While also printing out a numpyarray with the coordinates'''
...
def main()
    user_file_path = 'path/to/user_file_v2.txt'
    json_template_path = 'path/to/INDEL_TEMPLATE_v2.json'
    user_data, iq_model = read_user_data(user_file_path) #It reads the user data from the model output in the read_user_data() function
                                                         #The user_data is everything but the model, the model needed to be handled differently
    with open(json_template_path, 'r') as f:
        json_data = json.load(f)

    validate_user_data(user_data, json_data) #json_data is the combined user_file into the json template, user_data is the dict with all the user_file info in it
                                             #makes sure all the user_data aligns with the proper formatting indicated in the JSON  
    for i in range(1, 601):
        user_data['randomseed'] = random.randint(1, 100000)
        updated_json_data = update_json_structure(json_data, user_data, i) #this function places the user_data in the json data
        indel_input_path= f'{indelible_input_path}/indelible_input_{i}.txt'
        generate_config(updated_json_data, indel_input_path) #This generates 600 configuration files for INDELIBLE
                                                             #, 1-100 at the lowest GC content, and 500-600 at the highest GC content

    rename_indel(indel_input_path) #Here you input all the folder path where you'd like your indelible data to print out, I used the date/time module so it's precise
    indelible_output_path = f'path/to/indelible_output_{now_format}' #Here you need to select an indelible output path
    move_to_indel_output(indelible_input_path, indelible_output_path) #Indelible input_path is the same as the indel_input_path

    def rename_seq_fasta(indelible_output_path, IQTREE_path, IQTREE_exe_path, iq_model) #IQTREE installation path, IQTREE_exe = where the executable is located, iq_model is self chosen
    '''This function moves the correct files to iq_output folder, iterates over each file in the iq_output folder, 
       renames the sequence identifiers so they're unique (this is only if your file has repeat sequences 
       as they must be unique for IQ-TREE), and then runs each file through IQ-TREE.'''
