...
'''
def rename_indel(directory_path, indelible_src_directory, executable_path):
    Renames each Indelible input file to control.txt, runs Indelible, and renames it back.
    for i in range(1, 601):  # Assuming 600 input files
        original_path = os.path.join(directory_path, f"indelible_input_{i}.txt")
        print(original_path)
        temp_path = os.path.join(indelible_src_directory, "control.txt")
        print(temp_path)
        
        if os.path.exists(original_path):
            print(f"Moving {original_path} -> {temp_path}")
            shutil.move(original_path, temp_path)  

            print("Running Indelible...")
            os.chdir('/home/s36jshem_hpc/sealion/indel_install/INDELibleV1.03/src')
            command = f"./indelible control.txt"
            os.system(command)

            print(f"Moving {temp_path} back to {original_path}")
...
indelible_input_path = temp_path  
indel_dir = '/home/s36jshem_hpc/sealion/indel_install/INDELibleV1.03/src'
executable_path = '/home/s36jshem_hpc/sealion/indel_install/INDELibleV1.03/src/indelible'
'''
...
def make_clade_files(fasta_path, clade_path):
    ...
    for i in range(int(num_chunks)):
        output_file = os.path.join(clade_path, f"clade_file_{gc_contents[i]}_{i+1}.fas")
        ...
        clade_definition_file = os.path.join(clade_path, f"clade_def_file_{gc_contents[i]}_{i+1}.txt")    
        with open(clade_definition_file, 'w') as f:
            for group, seq_list in sequences.items():
                f.write(f'{group}, ')
                for q, seq in enumerate(sublist, start =start + 1):
...
clade_path = f'//home/s36jshem_hpc/sealion/runs/Clade_folder/clade_files_{now_format}'
